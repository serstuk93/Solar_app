...
class CalcSol:
    ...
    def calculations(self, positionAA):
        ...
        sunEqofCtr = sin(radians(geomMeanAnomSundeg)) * (1.914602 - jul_rok * (0.004817 + 0.000014 * jul_rok)) + sin(
            radians(2 * geomMeanAnomSundeg)) * (0.019993 - 0.000101 * jul_rok) + sin(
            radians(3 * geomMeanAnomSundeg)) * 0.000289
        ...
        sun_rad_vector_AUs = (1.000001018 * (1 - eccentEarthOrbit * eccentEarthOrbit)) / (
        ...
        mean_obliq_ecliptic_deg = 23 + (
                    26 + ((21.448 - jul_rok * (46.815 + jul_rok * (0.00059 - jul_rok * 0.001813)))) / 60) / 60
        obliq_corr_deg = mean_obliq_ecliptic_deg + 0.00256 * cos(radians(125.04 - 1934.136 * jul_rok))
        sun_rt_ascen_deg = degrees(
            atan2(cos(radians(obliq_corr_deg)) * sin(radians(sun_app_long_deg)), cos(radians(sun_app_long_deg))))
        print("!!!!!!!!",
              degrees(
                  atan2(
        ...
        eq_of_time_minutes = 4 * degrees(var_y * sin(2 * radians(geomMeanLongSundeg)) - 2 * eccentEarthOrbit * sin(
            radians(geomMeanAnomSundeg)) + 4 * eccentEarthOrbit * var_y * sin(radians(geomMeanAnomSundeg)) * cos(
            2 * radians(geomMeanLongSundeg)) - 0.5 * var_y * var_y * sin(
            4 * radians(geomMeanLongSundeg)) - 1.25 * eccentEarthOrbit * eccentEarthOrbit * sin(
            2 * radians(geomMeanAnomSundeg)))
        ha_sunrise_deg = degrees(acos(
            cos(radians(90.833)) / (cos(radians(location.latitude)) * cos(radians(sun_declin_deg))) - tan(
                radians(location.latitude)) * tan(radians(sun_declin_deg))))
        print(ha_sunrise_deg, "ha_sunrise_deg")
        print(
            sun_rad_vector_AUs, "sun_rad_vector_AUs OK ", sun_app_long_deg, "sun_app_long_deg OK",
            mean_obliq_ecliptic_deg, "mean_obliq_ecliptic_deg OK ",
        )
        print(
            obliq_corr_deg, "obliq_corr_deg", sun_rt_ascen_deg, "sun_rt_ascen_deg ", sun_declin_deg, "sun_declin_deg  ",
            var_y, "var_y  ",
        ...
        if true_solar_time_min / 4 < 0:
            ...
            hour_angle_deg = true_solar_time_min / 4 - 180
        solar_zenith_angle_deg = degrees(acos(
            sin(radians(location.latitude)) * sin(radians(sun_declin_deg)) + cos(radians(location.latitude)) * cos(
                radians(sun_declin_deg)) * cos(radians(hour_angle_deg))))
        ...
        if solar_elevation_angle_deg > 85:
            ...
            approx_atmospheric_refraction_deg = (58.1 / tan(radians(solar_elevation_angle_deg)) - 0.07 / pow(
                tan(radians(solar_elevation_angle_deg)), 3) + 0.000086 / pow(tan(radians(solar_elevation_angle_deg)),
                                                                             5)) / 3600
        elif solar_elevation_angle_deg > -0.575:
            approx_atmospheric_refraction_deg = (1735 + solar_elevation_angle_deg * (
                        -518.2 + solar_elevation_angle_deg * (
                            103.4 + solar_elevation_angle_deg * (-12.79 + solar_elevation_angle_deg * 0.711)))) / 3600
        else:
        ...
        if hour_angle_deg > 0:
            solar_azimuth_angle_deg_cwfrom_N = (degrees(acos(((sin(radians(location.latitude)) * cos(
                radians(solar_zenith_angle_deg))) - sin(radians(sun_declin_deg))) / (
                                                                         cos(radians(location.latitude)) * sin(
                                                                     radians(solar_zenith_angle_deg))))) + 180 % 360)
        else:
            solar_azimuth_angle_deg_cwfrom_N = (540 - degrees(acos(((sin(radians(location.latitude)) * cos(
                radians(solar_zenith_angle_deg))) - sin(radians(sun_declin_deg))) / (
                                                                               cos(radians(location.latitude)) * sin(
                                                                           radians(solar_zenith_angle_deg))))) % 360)
